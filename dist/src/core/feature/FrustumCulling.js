...
function _bvhCullingCallback(node) {
    const s = _bvhCullingState;
    const index = node.object;
    if (index < s.instancesArrayCount &&
...
function _bvhCullingLODSortCallback(node) {
...
function _bvhCullingLODCallback(node, level) {
...
const _lodIndexes = []; // Reusable array for LOD indexes
InstancedMesh2.prototype.getSectorAwarePosition = function (instanceId, target) {
    // Get local position
...
InstancedMesh2.prototype.getSectorOffsetFor = function (instanceId, target) {
    target.set(0, 0, 0);
    ...
    if (!perObjectFrustumCulled) {
        ...
        if (this.bvh) {
            if (this._hasSectors && this._globalTrackedSectorLow) {
                ...
                const wo = this._globalWorldOffset;
                _bvhTranslation.makeTranslation(-(tracked.x | 0) * SECTOR_SCALE - (wo?.x ?? 0), -(tracked.y | 0) * SECTOR_SCALE - (wo?.y ?? 0), -(tracked.z | 0) * SECTOR_SCALE - (wo?.z ?? 0));
...
InstancedMesh2.prototype.frustumCullingLOD = function (LODrenderList, camera, cameraLOD) {
    ...
    for (let i = 0; i < levels.length; i++) {
        _lodIndexes[i] = levels[i].object.instanceIndex.array;
    }
